...
void Skeet::animate()
{
   ...
   // spawn
   executor.execute(std::make_tuple("CREATE", 'B', time.level()));
...
void Skeet::interact(const UserInput & ui)
{
   ...
   if (ui.isSpace())
      executor.execute(std::make_tuple("FIRE", 'P', gun.getAngle()));
   // missiles can be shot at level 2 and higher
   else if (ui.isM() && time.level() > 1)
      executor.execute(std::make_tuple("FIRE", 'M', gun.getAngle()));
   // bombs can be shot at level 3 and higher
   else if (ui.isB() && time.level() > 2)
      executor.execute(std::make_tuple("FIRE", 'B', gun.getAngle()));

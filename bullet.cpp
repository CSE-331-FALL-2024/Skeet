...
Pellet::Pellet(double angle, double speed, Interface* iface, Logic* logic) 
    : Bullet(angle, speed, 1.0, 1)
{
    this->pIfaceElement = &iface->ifacePellet;
    this->pLogElement = &logic->logPellet;
}

Bomb::Bomb(double angle, double speed, Interface* iface, Logic* logic)
    : Bullet(angle, speed, 4.0, 4)
{
    this->pIfaceElement = &iface->ifaceBomb;
    this->pLogElement = &logic->logBomb;
    this->timeToDie = 60;
}

Missile::Missile(double angle, double speed, Interface* iface, Logic* logic)
    : Bullet(angle, speed, 1.0, 3) 
{
    this->pIfaceElement = &iface->ifaceMissile;
    this->pLogElement = &logic->logMissile;
}



Shrapnel::Shrapnel(const Bomb& bomb, Interface* iface, Logic* logic)
{
    this->pIfaceElement = &iface->ifaceBomb;
    this->pLogElement = &logic->logBomb;
